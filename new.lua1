...
uipallet = {
	Main = Color3.fromRGB(24, 27, 31),
	Text = Color3.fromRGB(237, 240, 243),
	Font = Font.fromEnum(Enum.Font.Gotham),
	FontSemiBold = Font.fromEnum(Enum.Font.Gotham, Enum.FontWeight.Medium),
...
function vape:CreateCategory(props)
	...
	if modern then modern:RegisterCategory(category, props) end
	return category
end
...
function vape:CreateNotification(title, text, duration, type)
	if modern then return modern:Notify(title, text, duration, type) end
...
function vape:Load(skipgui, profile)
	...
	if modern then modern:Refresh() end
	return toggleData
end
...
function vape:LoadGUI()
	...
	scale.Scale = math.clamp(gui.AbsoluteSize.X / 1280, 0.85, 1)
	...
	vape:Clean(scale:GetPropertyChangedSignal('Scale'):Connect(function()
		...
		if modern then modern:Layout(); return end
	...
	vape:Clean(inputService.InputBegan:Connect(function(input)
		if modern and modern:HandleInput(input) then return end
...
function vape:Remove(obj)
	if modern then modern:UnregisterModule(obj) end
...
function vape:Save(newProfile)
	if modern then modern:Save() end
...
function vape:SortCategories()
	...
	if modern then modern:QueueRefresh() end
end

function vape:Uninject()
	self:Save()
	if modern then modern:Destroy() end
	self.Loaded = nil
...
function vape:UpdateGUIQueue(hue, sat, val)
	...
	if modern then modern:UpdateAccent(hue, sat, val) end
...
components = {
	...
	Module = function(props, children, api)
		...
		for index, comp in components do
			component['Create'..index] = function(_, props)
				local option = comp(props, modulechildren, component)
				if modern then modern:OptionCreated(component, props and props.Name, option) end
				return option
			end
		end
		...
		if modern then modern:RegisterModule(component, props) end
		vape:SortCategories()
	...
	__newindex = function(_, index, callback)
		for _, module in vape.Modules do
			rawset(module, 'Create'..index, function(_, props)
				local option = callback(props, module.Children, module)
				if modern then modern:OptionCreated(module, props and props.Name, option) end
				return option
			end)
		end
...
local installModern = (function()
-- Modern presentation for Vape V4. The native component objects remain authoritative.
-- No game modules or simulated module data are included here.
return function(vape, env)
    ...
    local configPath = 'newvape/profiles/modern-ui.json'
    local themes = {
        Dark = {
            background = Color3.fromRGB(19, 21, 24), sidebar = Color3.fromRGB(16, 18, 20),
            panel = Color3.fromRGB(24, 27, 31), raised = Color3.fromRGB(32, 36, 42),
            line = Color3.fromRGB(43, 48, 54), text = Color3.fromRGB(237, 240, 243),
            muted = Color3.fromRGB(161, 170, 181), ink = Color3.fromRGB(16, 39, 31)
        },
        Light = {
            background = Color3.fromRGB(245, 246, 248), sidebar = Color3.fromRGB(237, 239, 242),
            panel = Color3.fromRGB(255, 255, 255), raised = Color3.fromRGB(229, 233, 238),
            line = Color3.fromRGB(213, 220, 228), text = Color3.fromRGB(27, 36, 44),
            muted = Color3.fromRGB(80, 94, 108), ink = Color3.fromRGB(255, 255, 255)
        }
    }
    local accents = {
        Mint = {Dark = Color3.fromRGB(113, 226, 181), Light = Color3.fromRGB(8, 125, 97)},
        Blue = {Dark = Color3.fromRGB(138, 185, 255), Light = Color3.fromRGB(39, 103, 190)},
        Violet = {Dark = Color3.fromRGB(189, 163, 255), Light = Color3.fromRGB(112, 69, 181)}
    }
    local settings = {Theme = 'Dark', Accent = 'Mint', Category = 'Render'}
    local saved = env.loadJson(configPath)
    if type(saved) == 'table' then
        if themes[saved.Theme] then settings.Theme = saved.Theme end
        if accents[saved.Accent] then settings.Accent = saved.Accent end
        if type(saved.Category) == 'string' then settings.Category = saved.Category end
    end
    local app = {
        Version = '1.0.0', Settings = settings, Records = {}, Categories = {},
        Selected = nil, Filter = 'All', Query = '', Compact = false,
        DetailOpen = false, Destroyed = false
    }
    local disposed = false
    local theme = themes[settings.Theme]
    local accent = accents[settings.Accent][settings.Theme]
    local bindings = setmetatable({}, {__mode = 'k'})
    local nativeSkinned = setmetatable({}, {__mode = 'k'})
    local uiConnections = {}
    local pendingRefresh = false
    local nativeWindows = setmetatable({}, {__mode = 'k'})
    local activeLegacy
    local toastItems = {}
    local font = Font.fromEnum(Enum.Font.Gotham)
    local medium = Font.fromEnum(Enum.Font.Gotham, Enum.FontWeight.Medium)
    -- These IDs are already bundled by the upstream GUI. They keep the modern
    -- shell visible when an executor does not expose getcustomasset reliably.
    local builtInAssets = {
        vapelogomini = 'rbxassetid://109041903452149',
        ...
        world = 'rbxassetid://118917453153459',
        inventory = 'rbxassetid://93264756888499',
        settings = 'rbxassetid://73820177347303',
        profiles = 'rbxassetid://126051451865127',
        friends = 'rbxassetid://92957214042038',
        search = 'rbxassetid://115611852955611'
    }
    ...
    local function connect(signal, callback, owner)
        ...
        table.insert(owner or uiConnections, connection)
        return connection
    end
    local function token(object, property, key)
        bindings[object] = bindings[object] or {}
        bindings[object][property] = key
        object[property] = key == 'accent' and accent or theme[key]
    end
    local function make(className, parent, props)
        ...
        if object:IsA('TextLabel') or object:IsA('TextButton') or object:IsA('TextBox') then
            ...
            object.TextSize = 14
            object.Text = ''
            token(object, 'TextColor3', 'text')
        end
        if object:IsA('GuiButton') then object.AutoButtonColor = false end
        for property, value in props or {} do object[property] = value end
        object.Parent = parent
        return object
    end
    local function corner(object, radius)
        return make('UICorner', object, {CornerRadius = UDim.new(0, radius or 10)})
    end
    local function stroke(object, key)
        local result = make('UIStroke', object, {Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border})
        token(result, 'Color', key or 'line')
        return result
    end
    local function panel(parent, name, key)
        local result = make('Frame', parent, {Name = name})
        token(result, 'BackgroundColor3', key or 'panel')
        return result
    end
    local function label(parent, text, size, key)
        local result = make('TextLabel', parent, {
            Text = text, TextSize = size or 14, BackgroundTransparency = 1,
            TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd
        })
        token(result, 'TextColor3', key or 'text')
        return result
    end
    local function textButton(parent, name, text, callback)
        local result = make('TextButton', parent, {Name = name, Text = text, TextSize = 12})
        token(result, 'BackgroundColor3', 'raised')
        corner(result, 8)
        if callback then connect(result.Activated, callback) end
        return result
    end
    local function icon(parent, path)
        local ok, image = pcall(env.asset, 'newvape/assets/new/'..path..'.png')
        if not ok then image = '' end
        if type(image) ~= 'string' or image == '' then image = builtInAssets[path] or '' end
        local result = make('ImageLabel', parent, {
            BackgroundTransparency = 1, Image = image,
            Size = UDim2.fromOffset(18, 18), ScaleType = Enum.ScaleType.Fit
        })
        token(result, 'ImageColor3', 'muted')
        return result
    end
    local function switch(parent, name, callback, owner)
        local button = make('TextButton', parent, {Name = name, Text = '', Size = UDim2.fromOffset(38, 22)})
        corner(button, 20)
        local knob = panel(button, 'Knob', 'muted')
        knob.Size = UDim2.fromOffset(14, 14)
        knob.Position = UDim2.fromOffset(4, 4)
        corner(knob, 20)
        connect(button.Activated, callback, owner)
        return {Object = button, Knob = knob}
    end
    local function paintSwitch(control, enabled)
        local changed = control.Object:GetAttribute('Enabled') ~= enabled
        control.Object.BackgroundColor3 = enabled and accent or theme.raised
        control.Knob.BackgroundColor3 = enabled and theme.ink or theme.muted
        local position = UDim2.fromOffset(enabled and 20 or 4, 4)
        if changed and env.tween then
            env.tween:Tween(control.Knob, TweenInfo.new(0.14, Enum.EasingStyle.Quad), {Position = position})
        else
            control.Knob.Position = position
        end
        control.Object:SetAttribute('Enabled', enabled)
    end
    local function scroll(parent, name)
        local result = make('ScrollingFrame', parent, {
            Name = name, BackgroundTransparency = 1, CanvasSize = UDim2.new(),
            AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollingDirection = Enum.ScrollingDirection.Y,
            ScrollBarThickness = 3, ClipsDescendants = true
        })
        token(result, 'ScrollBarImageColor3', 'muted')
        return result
    end
    local function skinNative(object)
        if nativeSkinned[object] then return end
        nativeSkinned[object] = true
        if object:IsA('GuiObject') then
            local _, saturation, brightness = object.BackgroundColor3:ToHSV()
            if saturation < 0.32 and object.BackgroundTransparency < 1 then
                local _, _, baseBrightness = palette.Main:ToHSV()
                local isTrack = object.Size.Y.Scale == 0 and object.Size.Y.Offset > 0 and object.Size.Y.Offset <= 4
                local raised = object:IsA('TextBox') or object.Name == 'Holder' or isTrack or brightness > baseBrightness + 0.025
                token(object, 'BackgroundColor3', raised and 'raised' or 'panel')
            end
            if object:IsA('TextLabel') or object:IsA('TextButton') or object:IsA('TextBox') then
                object.FontFace = font
                if object.TextSize < 12 then object.TextSize = 12 end
                local _, sat = object.TextColor3:ToHSV()
                if sat < 0.22 then token(object, 'TextColor3', 'text') end
                if object:IsA('TextBox') then token(object, 'PlaceholderColor3', 'muted') end
            end
        end
    end
    local function skinTree(object)
        skinNative(object)
        for _, child in object:GetDescendants() do skinNative(child) end
    end

    -- Keep original named windows in ClickGui for integrations that find them by name.
    -- Their controls and closures are reused, rather than recreated or serialized.
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    -- CanvasGroup is intentional: Roblox does not clip Frame descendants to
    -- UICorner edges, while CanvasGroup clips the whole window silhouette.
    local shell = make('CanvasGroup', clickgui, {Name = 'ModernWindow'})
    token(shell, 'BackgroundColor3', 'background')
    app.Object = shell
    shell.ZIndex = 2
    corner(shell, 18)
    stroke(shell)
    local top = panel(shell, 'Header', 'sidebar')
    top.Size = UDim2.new(1, 0, 0, 72)
    local drag = make('TextButton', top, {Name = 'DragHandle', Text = '', BackgroundTransparency = 1, Size = UDim2.new(1, -255, 1, 0)})
    local logo = icon(top, 'vapelogomini')
    logo.Position = UDim2.fromOffset(23, 25)
    logo.Size = UDim2.fromOffset(67, 21)
    token(logo, 'ImageColor3', 'text')
    local dot = panel(top, 'AccentDot', 'accent')
    dot.Position = UDim2.fromOffset(94, 39)
    dot.Size = UDim2.fromOffset(6, 6)
    corner(dot, 8)
    local version = label(top, 'V4', 11, 'muted')
    version.Position = UDim2.fromOffset(111, 26)
    version.Size = UDim2.fromOffset(28, 20)
    local profile = label(top, 'Workspace  /  default', 12, 'muted')
    profile.Position = UDim2.fromOffset(190, 25)
    profile.Size = UDim2.new(1, -470, 0, 23)
    local themeButton = textButton(top, 'Appearance', 'Light', function() app:SetTheme(settings.Theme == 'Dark' and 'Light' or 'Dark') end)
    themeButton.Position = UDim2.new(1, -156, 0, 20)
    themeButton.Size = UDim2.fromOffset(68, 32)
    local workspaceButton = textButton(top, 'WorkspaceMenu', 'Menu', function() app:OpenLegacy('Workspace') end)
    workspaceButton.Position = UDim2.new(1, -232, 0, 20)
    workspaceButton.Size = UDim2.fromOffset(68, 32)
    local close = textButton(top, 'Close', '×', function() vape.GUIBind.Triggered:Fire(true) end)
    close.Position = UDim2.new(1, -69, 0, 20)
    close.Size = UDim2.fromOffset(44, 32)
    close.TextSize = 22
    local topLine = panel(shell, 'HeaderDivider', 'line')
    topLine.Position = UDim2.fromOffset(0, 72)
    topLine.Size = UDim2.new(1, 0, 0, 1)
    local sidebar = panel(shell, 'Sidebar', 'sidebar')
    local sideTitle = label(sidebar, 'MODULES', 11, 'muted')
    sideTitle.Position = UDim2.fromOffset(22, 19)
    sideTitle.Size = UDim2.new(1, -30, 0, 20)
    local navigation = scroll(sidebar, 'Categories')
    navigation.Position = UDim2.fromOffset(10, 51)
    navigation.Size = UDim2.new(1, -20, 1, -244)
    local navLayout = make('UIListLayout', navigation, {Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder})
    local toolsArea = panel(sidebar, 'WorkspaceTools', 'sidebar')
    toolsArea.Position = UDim2.new(0, 10, 1, -180)
    toolsArea.Size = UDim2.new(1, -20, 0, 169)
    local toolLayout = make('UIListLayout', toolsArea, {Padding = UDim.new(0, 5), SortOrder = Enum.SortOrder.LayoutOrder})
    local main = panel(shell, 'Modules', 'background')
    local heading = label(main, settings.Category, 26)
    heading.FontFace = medium
    heading.Position = UDim2.fromOffset(22, 22)
    heading.Size = UDim2.new(1, -40, 0, 32)
    local count = label(main, '', 12, 'muted')
    count.Position = UDim2.fromOffset(22, 58)
    count.Size = UDim2.new(1, -40, 0, 20)
    local search = make('TextBox', main, {
        Name = 'Search', PlaceholderText = 'Search modules…', ClearTextOnFocus = false,
        TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left,
        Position = UDim2.fromOffset(22, 94), Size = UDim2.new(1, -178, 0, 38)
    })
    token(search, 'BackgroundColor3', 'panel')
    token(search, 'PlaceholderColor3', 'muted')
    corner(search, 9)
    stroke(search)
    make('UIPadding', search, {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 10)})
    local allButton = textButton(main, 'All', 'All', function() app:SetFilter('All') end)
    allButton.Size = UDim2.fromOffset(43, 32)
    allButton.Position = UDim2.new(1, -147, 0, 97)
    local enabledButton = textButton(main, 'Enabled', 'Enabled', function() app:SetFilter('Enabled') end)
    enabledButton.Size = UDim2.fromOffset(68, 32)
    enabledButton.Position = UDim2.new(1, -96, 0, 97)
    local cards = scroll(main, 'ModuleCards')
    cards.Position = UDim2.fromOffset(22, 149)
    cards.Size = UDim2.new(1, -42, 1, -165)
    make('UIPadding', cards, {PaddingRight = UDim.new(0, 4), PaddingBottom = UDim.new(0, 8), PaddingTop = UDim.new(0, 2), PaddingLeft = UDim.new(0, 2)})
    local grid = make('UIGridLayout', cards, {CellPadding = UDim2.fromOffset(12, 12), SortOrder = Enum.SortOrder.LayoutOrder})
    local empty = label(main, 'No modules match this view.', 14, 'muted')
    empty.Position = UDim2.fromOffset(22, 167)
    empty.Size = UDim2.new(1, -44, 0, 48)
    empty.TextWrapped = true
    local inspector = panel(shell, 'Inspector', 'panel')
    local inspectorLine = panel(inspector, 'Divider', 'line')
    inspectorLine.Size = UDim2.new(0, 1, 1, 0)
    local inspectorTitle = label(inspector, 'MODULE SETTINGS', 11, 'muted')
    inspectorTitle.Position = UDim2.fromOffset(18, 22)
    inspectorTitle.Size = UDim2.new(1, -76, 0, 20)
    local back = textButton(inspector, 'Back', 'Back', function() app.DetailOpen = false; app:Layout() end)
    back.Size = UDim2.fromOffset(48, 28)
    back.Position = UDim2.new(1, -66, 0, 18)
    local selectedName = label(inspector, 'Select a module', 19)
    selectedName.FontFace = medium
    selectedName.Position = UDim2.fromOffset(18, 56)
    selectedName.Size = UDim2.new(1, -80, 0, 29)
    local selectedToggle = switch(inspector, 'ToggleModule', function() if app.Selected then app.Selected.Module:Toggle() end end)
    selectedToggle.Object.Position = UDim2.new(1, -57, 0, 60)
    local description = label(inspector, 'Choose a module to view its settings.', 12, 'muted')
    description.Position = UDim2.fromOffset(18, 96)
    description.Size = UDim2.new(1, -36, 0, 57)
    description.TextWrapped = true
    description.TextTruncate = Enum.TextTruncate.None
    description.TextYAlignment = Enum.TextYAlignment.Top
    local options = scroll(inspector, 'Options')
    options.Position = UDim2.fromOffset(10, 164)
    options.Size = UDim2.new(1, -20, 1, -298)
    local noOptions = label(inspector, 'This module has no additional settings.', 12, 'muted')
    noOptions.TextWrapped = true
    noOptions.Position = UDim2.fromOffset(18, 172)
    noOptions.Size = UDim2.new(1, -36, 0, 50)
    local bottom = panel(inspector, 'InspectorFooter', 'panel')
    bottom.Position = UDim2.new(0, 18, 1, -124)
    bottom.Size = UDim2.new(1, -36, 0, 116)
    local bottomLine = panel(bottom, 'Divider', 'line')
    bottomLine.Size = UDim2.new(1, 0, 0, 1)
    local bindLabel = label(bottom, 'Keybind', 12, 'muted')
    bindLabel.Position = UDim2.fromOffset(0, 13)
    bindLabel.Size = UDim2.fromOffset(70, 28)
    local bindButton = textButton(bottom, 'Keybind', 'Unbound', function() app:BeginBind() end)
    bindButton.Position = UDim2.new(1, -133, 0, 11)
    bindButton.Size = UDim2.fromOffset(133, 30)
    bindButton.TextTruncate = Enum.TextTruncate.AtEnd
    local resetButton = textButton(bottom, 'Reset', 'Reset settings', function() app:ResetSelected() end)
    resetButton.Position = UDim2.fromOffset(0, 54)
    resetButton.Size = UDim2.new(1, 0, 0, 33)
    local bindHint = label(bottom, 'Shift + click toggles hold binding', 10, 'muted')
    bindHint.Position = UDim2.fromOffset(0, 92)
    bindHint.Size = UDim2.new(1, 0, 0, 18)
    local footer = panel(shell, 'Footer', 'sidebar')
    footer.Position = UDim2.new(0, 0, 1, -38)
    footer.Size = UDim2.new(1, 0, 0, 38)
    local footerLine = panel(footer, 'Divider', 'line')
    footerLine.Size = UDim2.new(1, 0, 0, 1)
    local active = label(footer, '0 modules enabled', 11, 'muted')
    active.Position = UDim2.fromOffset(22, 9)
    active.Size = UDim2.new(0.6, -22, 0, 20)
    local hotkey = label(footer, 'RSHIFT  Toggle UI    /  Search', 11, 'muted')
    hotkey.TextXAlignment = Enum.TextXAlignment.Right
    hotkey.Position = UDim2.fromScale(0.6, 0)
    hotkey.Size = UDim2.new(0.4, -22, 1, 0)
    local storage = panel(clickgui, 'ModernOptionStorage', 'panel')
    storage.Visible = false
    storage.Size = UDim2.fromOffset(234, 600)
    local legacy = make('Frame', clickgui, {Name = 'ModernDialogs', BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), Visible = false, ZIndex = 40})
    local dismiss = make('TextButton', legacy, {Name = 'Dismiss', Text = '', BackgroundColor3 = Color3.new(), BackgroundTransparency = 0.45, Size = UDim2.fromScale(1, 1), Modal = true})
    local dialogMount = scroll(legacy, 'DialogContent')
    dialogMount.ZIndex = 2
    dialogMount.AutomaticCanvasSize = Enum.AutomaticSize.None
    local dialogClose = textButton(legacy, 'CloseDialog', 'Close', function() app:CloseLegacy() end)
    dialogClose.ZIndex = 3
    dialogClose.Size = UDim2.fromOffset(70, 32)
    connect(dismiss.Activated, function() app:CloseLegacy() end)
    local toastLayer = make('Frame', env.scaledgui, {Name = 'ModernNotifications', BackgroundTransparency = 1, Size = UDim2.fromScale(1, 1), ZIndex = 60})

    function app:Save()
        if disposed then return end
        local ok, err = pcall(function() writefile(configPath, env.http:JSONEncode(settings)) end)
        if not ok then warn('Vape Modern: could not save appearance: '..tostring(err)) end
    end
    function app:QueueRefresh()
        if pendingRefresh or disposed then return end
        pendingRefresh = true
        task.defer(function()
            pendingRefresh = false
            if not disposed then self:Refresh() end
        end)
    end
    function app:RefreshDetails()
        local record = self.Selected
        selectedToggle.Object.Visible = record ~= nil
        bindButton.Visible = record ~= nil
        resetButton.Visible = record ~= nil
        bindHint.Visible = record ~= nil
        if not record then
            selectedName.Text = 'Select a module'
            description.Text = 'Choose a module to view its settings.'
            noOptions.Visible = false
            return
        end
        local module = record.Module
        selectedName.Text = module.Name
        description.Text = record.Description
        paintSwitch(selectedToggle, module.Enabled)
        local bind = module.Bind
        bindButton.Text = vape.Binding == bind and 'Press a key…' or (#bind.Keys > 0 and table.concat(bind.Keys, ' + ') or 'Unbound')
        bindHint.Text = bind.Hold and 'Hold binding · Shift + click to change' or 'Shift + click toggles hold binding'
        local hasOption = false
        for _, child in module.Children:GetChildren() do
            if child:IsA('GuiObject') and child.Visible then hasOption = true; break end
        end
        noOptions.Visible = not hasOption
    end
    function app:Select(record, openDetail)
        if self.Selected and self.Selected ~= record then
            self.Selected.Module.Children.Visible = false
            self.Selected.Module.Children.Parent = storage
        end
        self.Selected = record
        if record then
            record.Module.Children.Parent = options
            record.Module.Children.Position = UDim2.new()
            record.Module.Children.Visible = true
            skinTree(record.Module.Children)
            -- Original controls already compute their height from their list layout.
            options.CanvasSize = UDim2.fromOffset(0, record.Module.Children.Size.Y.Offset)
        end
        if openDetail and self.Compact then self.DetailOpen = true end
        options.CanvasPosition = Vector2.new()
        self:RefreshDetails()
        self:Refresh()
        self:Layout()
    end
    function app:SetCategory(name)
        ...
        for _, record in self.Records do
            if record.Module.Category == name and record.Module.Visible ~= false and record.Module.Object.Visible then
                table.insert(candidates, record)
            end
        end
        table.sort(candidates, function(a, b) return a.Module.Name:lower() < b.Module.Name:lower() end)
        self:Select(candidates[1], false)
        cards.CanvasPosition = Vector2.new()
        self:Save()
    end
    function app:SetFilter(value)
        self.Filter = value == 'Enabled' and 'Enabled' or 'All'
        self:Refresh()
    end
    function app:SetQuery(value)
        self.Query = tostring(value):lower()
        self:Refresh()
    end
    function app:Refresh()
        if disposed then return end
        local totals, enabledCounts = {}, {}
        local enabledTotal, visibleCount = 0, 0
        for _, record in self.Records do
            local module = record.Module
            totals[module.Category] = (totals[module.Category] or 0) + 1
            if module.Enabled then
                enabledCounts[module.Category] = (enabledCounts[module.Category] or 0) + 1
                enabledTotal += 1
            end
            local visible = module.Category == settings.Category and module.Visible ~= false and module.Object.Visible
                and (self.Filter == 'All' or module.Enabled)
                and (module.Name..' '..record.Description):lower():find(self.Query, 1, true) ~= nil
            record.Card.Visible = visible
            record.Card.LayoutOrder = module.Object.LayoutOrder
            if visible then visibleCount += 1 end
            local selected = self.Selected == record
            record.Card.BackgroundColor3 = selected and theme.panel:Lerp(accent, 0.09) or theme.panel
            record.Stroke.Color = selected and theme.line:Lerp(accent, 0.65) or theme.line
            record.Icon.ImageColor3 = module.Enabled and accent or theme.muted
            paintSwitch(record.Switch, module.Enabled)
        end
        for name, category in self.Categories do
            category.Count.Text = tostring(totals[name] or 0)
            local selected = name == settings.Category
            category.Button.BackgroundTransparency = selected and 0 or 1
            category.Button.BackgroundColor3 = theme.sidebar:Lerp(accent, 0.10)
            category.Label.TextColor3 = selected and accent or theme.muted
            category.Icon.ImageColor3 = category.Label.TextColor3
        end
        heading.Text = settings.Category
        count.Text = string.format('%d modules · %d enabled', totals[settings.Category] or 0, enabledCounts[settings.Category] or 0)
        active.Text = string.format('%d modules enabled  ·  %s', enabledTotal, vape.Profile or 'default')
        profile.Text = 'Workspace  /  '..(vape.Profile or 'default')
        empty.Visible = visibleCount == 0
        empty.Text = (totals[settings.Category] or 0) == 0 and 'No modules in this category yet.' or 'No modules match this view.'
        allButton.BackgroundColor3 = self.Filter == 'All' and theme.raised or theme.background
        enabledButton.BackgroundColor3 = self.Filter == 'Enabled' and theme.raised or theme.background
        self:RefreshDetails()
    end
    function app:Layout()
        if disposed then return end
        local width = math.max(280, math.min(1024, gui.AbsoluteSize.X / scale.Scale - 24))
        local height = math.max(380, math.min(740, gui.AbsoluteSize.Y / scale.Scale - 32))
        shell.Size = UDim2.fromOffset(width, height)
        if not self.Dragged then
            shell.Position = UDim2.fromOffset(math.max(0, (gui.AbsoluteSize.X / scale.Scale - width) / 2), math.max(0, (gui.AbsoluteSize.Y / scale.Scale - height) / 2))
        else
            shell.Position = UDim2.fromOffset(math.clamp(shell.Position.X.Offset, 0, math.max(0, gui.AbsoluteSize.X / scale.Scale - width)), math.clamp(shell.Position.Y.Offset, 0, math.max(0, gui.AbsoluteSize.Y / scale.Scale - height)))
        end
        self.Compact = width < 680
        local sideWidth = width >= 900 and 166 or 140
        local inspectorWidth = 254
        local contentHeight = height - 111
        local optionsTop = height < 570 and 122 or 164
        description.Size = UDim2.new(1, -36, 0, height < 570 and 22 or 57)
        options.Position = UDim2.fromOffset(10, optionsTop)
        options.Size = UDim2.new(1, -20, 1, -(optionsTop + 134))
        noOptions.Position = UDim2.fromOffset(18, optionsTop + 8)
        local mainWidth
        profile.Visible = width >= 900
        version.Visible = width >= 460
        logo.Visible = width >= 380
        dot.Visible = width >= 380
        back.Visible = self.Compact
        hotkey.Visible = width >= 680
        active.Size = UDim2.new(self.Compact and 1 or 0.6, -24, 0, 20)
        if self.Compact then
            sidebar.Position = UDim2.fromOffset(0, 73)
            sidebar.Size = UDim2.new(1, 0, 0, 87)
            sideTitle.Visible = false
            toolsArea.Visible = false
            navigation.Position = UDim2.fromOffset(12, 7)
            navigation.Size = UDim2.new(1, -24, 0, 75)
            navLayout.FillDirection = Enum.FillDirection.Horizontal
            navigation.ScrollingDirection = Enum.ScrollingDirection.X
            navigation.AutomaticCanvasSize = Enum.AutomaticSize.X
            main.Position = UDim2.fromOffset(0, 160)
            main.Size = UDim2.new(1, 0, 1, -198)
            main.Visible = not self.DetailOpen
            inspector.Position = UDim2.fromOffset(0, 73)
            inspector.Size = UDim2.new(1, 0, 1, -111)
            inspector.Visible = self.DetailOpen
            sidebar.Visible = not self.DetailOpen
            mainWidth = width
            for _, category in self.Categories do
                category.Button.Size = UDim2.fromOffset(133, 52)
                category.Count.Visible = false
                category.Label.Size = UDim2.new(1, -44, 1, 0)
            end
        else
            ...
            sidebar.Position = UDim2.fromOffset(0, 73)
            sidebar.Size = UDim2.fromOffset(sideWidth, contentHeight)
            sideTitle.Visible = true
            toolsArea.Visible = true
            navigation.Position = UDim2.fromOffset(10, 51)
            navigation.Size = UDim2.new(1, -20, 1, -244)
            navLayout.FillDirection = Enum.FillDirection.Vertical
            navigation.ScrollingDirection = Enum.ScrollingDirection.Y
            navigation.AutomaticCanvasSize = Enum.AutomaticSize.Y
            mainWidth = width - sideWidth - inspectorWidth
            main.Position = UDim2.fromOffset(sideWidth, 73)
            main.Size = UDim2.fromOffset(mainWidth, contentHeight)
            inspector.Position = UDim2.fromOffset(width - inspectorWidth, 73)
            inspector.Size = UDim2.fromOffset(inspectorWidth, contentHeight)
            for _, category in self.Categories do
                category.Button.Size = UDim2.new(1, -2, 0, 40)
                category.Count.Visible = true
                category.Label.Size = UDim2.new(1, -55, 1, 0)
            end
        end
        local columns = mainWidth >= 405 and 2 or 1
        grid.CellSize = UDim2.new(1 / columns, columns == 2 and -10 or -8, 0, columns == 2 and 140 or 112)
        local smallToolbar = mainWidth < 330
        search.Size = UDim2.new(1, smallToolbar and -44 or -178, 0, 38)
        allButton.Position = smallToolbar and UDim2.fromOffset(22, 141) or UDim2.new(1, -147, 0, 97)
        enabledButton.Position = smallToolbar and UDim2.fromOffset(73, 141) or UDim2.new(1, -96, 0, 97)
        cards.Position = UDim2.fromOffset(22, smallToolbar and 190 or 149)
        cards.Size = UDim2.new(1, -42, 1, smallToolbar and -206 or -165)
        empty.Position = UDim2.fromOffset(22, smallToolbar and 199 or 164)
        dialogMount.Size = UDim2.fromOffset(math.min(300, width - 28), math.max(150, height - 104))
        dialogMount.Position = UDim2.fromOffset((gui.AbsoluteSize.X / scale.Scale - dialogMount.Size.X.Offset) / 2, (gui.AbsoluteSize.Y / scale.Scale - dialogMount.Size.Y.Offset) / 2)
        dialogClose.Position = dialogMount.Position + UDim2.fromOffset(dialogMount.Size.X.Offset - 70, -38)
    end
    function app:RegisterCategory(component, props)
        if component.Type ~= 'Category' or self.Categories[component.Name] then return end
        local name = component.Name
        local order = {Combat = 1, Blatant = 2, Render = 3, Utility = 4, World = 5, Inventory = 6}
        local nav = textButton(navigation, name, '', function() self:SetCategory(name) end)
        nav.LayoutOrder = order[name] or 20
        nav.Size = UDim2.new(1, -2, 0, 40)
        local image = make('ImageLabel', nav, {
            BackgroundTransparency = 1, Position = UDim2.fromOffset(11, 12),
            Size = UDim2.fromOffset(16, 16),
            Image = props and props.Icon ~= '' and props.Icon or builtInAssets[name:lower()] or '',
            ScaleType = Enum.ScaleType.Fit
        })
        if image.Image == '' then
            local existing = component.Object:FindFirstChildWhichIsA('ImageLabel')
            image.Image = existing and existing.Image or ''
        end
        local text = label(nav, name, 13, 'muted')
        text.Position = UDim2.fromOffset(37, 0)
        text.Size = UDim2.new(1, -62, 1, 0)
        local number = label(nav, '0', 11, 'muted')
        number.TextXAlignment = Enum.TextXAlignment.Right
        number.Position = UDim2.new(1, -24, 0, 0)
        number.Size = UDim2.fromOffset(14, 40)
        self.Categories[name] = {Component = component, Button = nav, Label = text, Icon = image, Count = number}
        component.Object.Visible = false
        connect(component.Object:GetPropertyChangedSignal('Visible'), function()
            if component.Object.Visible then component.Object.Visible = false end
        end)
        self:QueueRefresh()
    end
    function app:RegisterModule(module, props)
        if self.Records[module.Name] then self:UnregisterModule(module.Name) end
        local record = {Module = module, Defaults = {}, Connections = {}, Description = tostring(props.Tooltip or ''):gsub('<[^>]+>', '')}
        local card = panel(cards, module.Name, 'panel')
        record.Card = card
        corner(card, 13)
        record.Stroke = stroke(card)
        local category = self.Categories[module.Category]
        record.Icon = make('ImageLabel', card, {BackgroundTransparency = 1, Image = category and category.Icon.Image or '', Position = UDim2.fromOffset(17, 18), Size = UDim2.fromOffset(18, 18), ScaleType = Enum.ScaleType.Fit})
        record.Switch = switch(card, 'Toggle', function() module:Toggle() end, record.Connections)
        record.Switch.Object.Position = UDim2.new(1, -55, 0, 16)
        local open = make('TextButton', card, {Name = 'OpenSettings', Text = '', BackgroundTransparency = 1, Position = UDim2.fromOffset(0, 49), Size = UDim2.new(1, 0, 1, -49)})
        local name = label(open, module.Name, 14)
        name.FontFace = medium
        name.Position = UDim2.fromOffset(17, 0)
        name.Size = UDim2.new(1, -44, 0, 23)
        local chevron = label(open, '›', 19, 'muted')
        chevron.Position = UDim2.new(1, -26, 0, 0)
        chevron.Size = UDim2.fromOffset(16, 22)
        local summary = label(open, record.Description, 12, 'muted')
        summary.TextWrapped = true
        summary.TextYAlignment = Enum.TextYAlignment.Top
        summary.Position = UDim2.fromOffset(17, 29)
        summary.Size = UDim2.new(1, -32, 1, -35)
        connect(open.Activated, function() self:Select(record, true) end, record.Connections)
        connect(card.InputBegan, function(event)
            if event.UserInputType == Enum.UserInputType.MouseButton2 then self:Select(record, true) end
        end, record.Connections)
        module.Children.Parent = storage
        module.Children.Visible = false
        skinTree(module.Children)
        connect(module.Children.DescendantAdded, function(child) skinNative(child); self:QueueRefresh() end, record.Connections)
        connect(module.Children:GetPropertyChangedSignal('Size'), function()
            if self.Selected == record then options.CanvasSize = UDim2.fromOffset(0, module.Children.Size.Y.Offset) end
        end, record.Connections)
        connect(module.Object:GetPropertyChangedSignal('Visible'), function() self:QueueRefresh() end, record.Connections)
        connect(module.Object:GetPropertyChangedSignal('LayoutOrder'), function() self:QueueRefresh() end, record.Connections)
        local originalSetBind = module.Bind.SetBind
        module.Bind.SetBind = function(bind, ...)
            originalSetBind(bind, ...)
            if not disposed then self:RefreshDetails() end
        end
        self.Records[module.Name] = record
        if not self.Selected and module.Category == settings.Category then self:Select(record, false) end
        self:QueueRefresh()
    end
    function app:OptionCreated(module, name, option)
        local record = self.Records[module.Name]
        if not record then return end
        if option and option.Save and name then
            local data = {}
            option:Save(data)
            for key, value in data do record.Defaults[key] = copy(value) end
        end
        self:QueueRefresh()
    end
    function app:UnregisterModule(name)
    ...
    function app:BeginBind()
        if not self.Selected then return end
        local bind = self.Selected.Module.Bind
        if input:IsKeyDown(Enum.KeyCode.LeftShift) then
            bind.Hold = not bind.Hold
        elseif vape.Binding == bind then
            bind:SetBind(copy(bind.Keys))
            vape.Binding = nil
        elseif not vape.Binding then
            bind.Binding = true
            vape.Binding = bind
        end
        self:RefreshDetails()
    ...
    function app:HandleInput(event)
        if not clickgui.Visible then return false end
        if vape.Binding and (event.KeyCode == Enum.KeyCode.Escape or event.KeyCode == Enum.KeyCode.Delete or event.KeyCode == Enum.KeyCode.Backspace) then
            local bind = vape.Binding
            bind:SetBind(event.KeyCode == Enum.KeyCode.Escape and copy(bind.Keys) or {})
            vape.Binding = nil
            self:RefreshDetails()
            return true
        end
        if input:GetFocusedTextBox() then return false end
        if event.KeyCode == Enum.KeyCode.Slash and not vape.Binding then search:CaptureFocus(); return true end
        if event.KeyCode == Enum.KeyCode.Escape and legacy.Visible then self:CloseLegacy(); return true end
        return false
    end
    function app:ResetSelected()
        if not self.Selected then return end
        vape:LoadOptions(self.Selected.Module, copy(self.Selected.Defaults))
        self.Selected.Module.Bind:SetBind({})
        if vape.Binding == self.Selected.Module.Bind then vape.Binding = nil end
        self:RefreshDetails()
        self:Notify('Settings reset', self.Selected.Module.Name..' options restored to defaults.', 3)
    end
    function app:SetTheme(name)
        if not themes[name] then return end
        settings.Theme = name
        theme = themes[name]
        accent = accents[settings.Accent][name]
        palette.Main = theme.panel
        palette.Text = theme.text
        for object, properties in bindings do
            if object.Parent then
                for property, key in properties do object[property] = key == 'accent' and accent or theme[key] end
            end
        end
        themeButton.Text = name == 'Dark' and 'Light' or 'Dark'
        local h, s, v = accent:ToHSV()
        vape.GUIColor:SetValue(h, s, v)
        self:Refresh()
        self:Save()
    end
    function app:SetAccent(name)
        if not accents[name] then return end
        settings.Accent = name
        self:SetTheme(settings.Theme)
    end
    function app:UpdateAccent(h, s, v)
        accent = Color3.fromHSV(h, s, v)
        dot.BackgroundColor3 = accent
        self:Refresh()
    end
    function app:CloseLegacy()
        if activeLegacy then
            local window = activeLegacy
            activeLegacy = nil
            window.Visible = false
            window.Parent = clickgui
        end
    ...
    function app:OpenLegacy(name)
        self:CloseLegacy()
        local category = (name == 'Settings' or name == 'Workspace') and vape.Categories.Main or vape.Categories[name]
        if not category then return end
        local window = category.Object
        activeLegacy = window
        window.Parent = dialogMount
        window.Position = UDim2.fromOffset(12, 4)
        window.Visible = true
        if category.Type == 'CategoryList' and not category.Expanded then category:Expand() end
        if name == 'Settings' then category.Settings.Object.Visible = true end
        if name == 'Workspace' then category.Settings.Object.Visible = false end
        skinTree(window)
        dialogMount.CanvasSize = UDim2.fromOffset(0, math.max(window.Size.Y.Offset + 20, 620))
        legacy.Visible = true
        self:Layout()
    end
    function app:Notify(title, text, duration, kind)
        if disposed or (vape.Notifications and not vape.Notifications.Enabled) then return end
        duration = math.clamp(tonumber(duration) or 4, 0.5, 30)
        local note = panel(toastLayer, 'Notification', 'panel')
        note.AnchorPoint = Vector2.new(1, 1)
        note.Size = UDim2.fromOffset(math.min(340, gui.AbsoluteSize.X / scale.Scale - 24), 88)
        corner(note, 12)
        stroke(note)
        local line = panel(note, 'Progress', 'accent')
        line.Position = UDim2.new(0, 12, 1, -5)
        line.Size = UDim2.new(1, -24, 0, 2)
        if kind == 'alert' then line.BackgroundColor3 = Color3.fromRGB(242, 109, 109) end
        if kind == 'warning' then line.BackgroundColor3 = Color3.fromRGB(255, 184, 121) end
        local titleLabel = label(note, tostring(title), 14)
        titleLabel.RichText = true
        titleLabel.FontFace = medium
        titleLabel.Position = UDim2.fromOffset(15, 12)
        titleLabel.Size = UDim2.new(1, -30, 0, 23)
        local body = label(note, tostring(text), 12, 'muted')
        body.RichText = true
        body.TextWrapped = true
        body.Position = UDim2.fromOffset(15, 39)
        body.Size = UDim2.new(1, -30, 0, 34)
        local now = os.clock()
        table.insert(toastItems, {Object = note, Progress = line, Start = now, Duration = duration})
        while #toastItems > 4 do table.remove(toastItems, 1).Object:Destroy() end
    end
    function app:Destroy()
        ...
        disposed = true
        self.Destroyed = true
        for _, connection in uiConnections do connection:Disconnect() end
        for _, record in self.Records do
            for _, connection in record.Connections do connection:Disconnect() end
        end
        table.clear(self.Records)
        table.clear(toastItems)
    end

    local function registerWindow(child)
        if not child:IsA('GuiObject') or child == shell or child == storage or child == legacy then return end
        if nativeWindows[child] then return end
        nativeWindows[child] = true
        local suppress = child.Name == 'GUICategory' or child.Name == 'Search' or child:IsA('TextLabel')
        for _, category in vape.Categories do
            if category.Type == 'Category' and category.Object == child then suppress = true end
        end
        child.ZIndex = 50
        if suppress then
            child.Visible = false
            connect(child:GetPropertyChangedSignal('Visible'), function()
                if child.Visible and child ~= activeLegacy then child.Visible = false end
            end)
        end
    end
    for _, child in clickgui:GetChildren() do registerWindow(child) end
    connect(clickgui.ChildAdded, registerWindow)
    connect(clickgui:GetPropertyChangedSignal('Visible'), function()
        if not clickgui.Visible then app:CloseLegacy() end
        app:QueueRefresh()
    end)
    connect(search:GetPropertyChangedSignal('Text'), function() app:SetQuery(search.Text) end)
    connect(gui:GetPropertyChangedSignal('AbsoluteSize'), function() app:Layout() end)
    connect(scale:GetPropertyChangedSignal('Scale'), function() app:Layout() end)
    local dragging, dragStart, windowStart
    connect(drag.InputBegan, function(event)
    ...
    connect(input.InputChanged, function(event)
        if dragging and (event.UserInputType == Enum.UserInputType.MouseMovement or event == dragging) then
            local delta = (Vector2.new(event.Position.X, event.Position.Y) - dragStart) / scale.Scale
            shell.Position = windowStart + UDim2.fromOffset(delta.X, delta.Y)
            app:Layout()
        end
    end)
    connect(input.InputEnded, function(event) if event == dragging then dragging = nil end end)
    local elapsed = 0
    connect(runService.Heartbeat, function(delta)
        ...
        elapsed = 0
        if activeLegacy then
            local height = activeLegacy.Size.Y.Offset
            for _, pane in vape.Settings do
                if pane.Object.Visible then height = math.max(height, pane.Object.Size.Y.Offset + 12) end
            end
            dialogMount.CanvasSize = UDim2.fromOffset(0, height + 20)
        end
        local now = os.clock()
        for index = #toastItems, 1, -1 do
            local item = toastItems[index]
            if now - item.Start >= item.Duration then
                item.Object:Destroy()
                table.remove(toastItems, index)
            end
        end
        for index, item in toastItems do
            item.Object.Position = UDim2.new(1, -16, 1, -16 - (#toastItems - index) * 98)
            item.Progress.Size = UDim2.fromOffset(math.max(0, (item.Object.Size.X.Offset - 24) * (1 - (now - item.Start) / item.Duration)), 2)
        end
    end)
    local toolNames = {'Settings', 'Profiles', 'Friends', 'Targets'}
    for index, name in toolNames do
        local button = textButton(toolsArea, name, name, function() app:OpenLegacy(name) end)
        button.Size = UDim2.new(1, -2, 0, 35)
        button.LayoutOrder = index
        button.BackgroundTransparency = 1
        button.TextXAlignment = Enum.TextXAlignment.Left
        make('UIPadding', button, {PaddingLeft = UDim.new(0, 11)})
        token(button, 'TextColor3', 'muted')
    end
    -- Keep the native alternate menu reachable after hiding the floating search bar.
    vape.Categories.Main:CreateGUIButton({
        Name = 'Legit mode',
        Function = function()
            app:CloseLegacy()
            vape.Legit.Window.Visible = true
    ...
    for _, category in vape.Categories do app:RegisterCategory(category) end
    for _, module in vape.Modules do app:RegisterModule(module, {}) end
    app:SetTheme(settings.Theme)
    app:Layout()
    app:Refresh()
    vape:Clean(function() app:Destroy() end)
    return app
end
...
vape:LoadGUI()
modern = installModern(vape, {
    input = inputService, runService = runService, clickgui = clickgui,
    gui = gui, scaledgui = scaledgui, scale = scale, palette = uipallet,
    color = color, asset = getvapeasset, loadJson = loadJson, http = httpService, tween = tween
})
vape.ModernUI = modern
